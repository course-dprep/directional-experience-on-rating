...
Highly popular movies (many votes) might show different rating behavior compared to less-known films.
```{r}
# Relatie voor aantal stemmen en rating
...
This histogram focuses on typical movie lengths (0–300 minutes), giving insight into runtime trends.
```{r}
# Filter complete cases en focus op normale speelduur (0-300 min)
imdb_runtime <- imdb_complete %>%
  filter(runtimeMinutes > 0, runtimeMinutes <= 300)

# Histogram van filmlengtes
...
- The IMDb dataset contains information on several thousand movies, covering multiple attributes such as ratings, release year, runtime, and popularity.
- Most movies have ratings clustered around a central value (often between 6 and 7).
- Average ratings appear relatively stable across years, though minor trends may exist.
...
This dataset provides a valuable foundation for further analysis, such as predicting movie success, exploring genre effects, or studying temporal changes in audience preferences.